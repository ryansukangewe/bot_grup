...
const { imageToWebp, imageToWebp3, videoToWebp, writeExifImg, writeExifImgAV, writeExifVid } = require('./exif')
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, await, sleep } = require('./myfunc2')
...
const { state, saveCreds } = await useMultiFileAuthState(`./data/fernazer-jadibot/`)
try {
...
require('../FliXBOT')(client[from], m, chatUpdate, mek, store)
}
} catch (err) {
console.log(err)
}
...
async function stopjadibot(rafatharcode, m, from) {
  ...
  delete client[from]
  rimraf.sync(`./data/fernazer-jadibot/${from.split("@")[0]}`)
}
